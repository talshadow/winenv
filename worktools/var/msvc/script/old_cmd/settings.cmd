...
set CMAKE_FOR_MSVC_RELEASE=-DCMAKE_BUILD_TYPE=Release -G "NMake Makefiles"
set zlib_settings=../zlib_src -DCMAKE_INSTALL_PREFIX=%LIB_DIR%\zlib %CMAKE_FOR_MSVC_RELEASE%
set zlib_ver=1.2.8

set lpng_settings= ../lpng_src -DCMAKE_INSTALL_PREFIX=%LIB_DIR%\lpng %CMAKE_FOR_MSVC_RELEASE%
set lpng_ver=162

set sqlite_settings=--prefix=%MLIB_DIR%/sqlite --enable-threadsafe 
rem %SYSTEM_CROSS_PARAM%
set sqlite_ver=autoconf-3071700

set icu_settings=Cygwin/MSVC --prefix=%MLIB_DIR%/icu --disable-tests --disable-samples --enable-shared --disable-static --enable-auto-cleanup 
rem %SYSTEM_CROSS_PARAM%
set icu_ver=
...
set boost_settings=%boost_settings% toolset=msvc
...
set boost_settings_release=%boost_settings% release stage
rem cxxflags="%CXXFLAGS%" --std=c++0x stage
